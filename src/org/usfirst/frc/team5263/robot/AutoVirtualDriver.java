...
public class AutoVirtualDriver {
	...
	public void periodicAuto() {
		 ...
		 * System.out.println(step); autoRunner = 2; break;
		 * 
		 * 
		 * 
		 ...
		 * 
		 * 
		 * case 3: encoder1Val = sensing.getEncoder1();
		 * System.out.println("running drive"); System.out.println("step " +
		 * step + " steps " + steps); encoderSet = encoderSet + 1; if(encoderSet
		 * == 1){ sensing.encoder1.reset(); encoderMin = distance[step] - 50;
		 * encoderMax = distance[step] + 50; } if(encoder1Val < encoderMin){
		 ...
		 * manipulators.myRobot.tankDrive(-0.4, -0.4); }else{ if(step < steps -
		 * 1){ //whats happening is that this is repeated over and over and
		 * always defaults to the else, and the condition is true step = step +
		 * 1; System.out.println("STEP HIGHER"); autoRunner = 4;
		 * 
		 * } }
		 * 
		 * 
		 ...
		 * angleOffset); System.out.println("first angle: " + firstAngle); rep =
		 * rep + 1; System.out.println("counter: " + rep); double angle =
		 * angleOffset - firstAngle;
		 * 
		 * 
		 * if(rep == 1){ angle = angle - angleOffset; }
		 **/
		 ...
		 * manipulators.myRobot.tankDrive(0.4, 0.4); System.out.println("driving
		 * straght"); }
		 * 
		 * }
		 **/
		 ...
		 * System.out.println("slow max"); } System.out.println("angle " + angle
		 * + " more than specified " + turn[step]); }else{
		 * System.out.println("finished turn steps"); after = after + 1;
		 * if(after == 1){ afterTurn = encoder1Val; }
		 ...
		 * distance[step] + afterTurn); manipulators.myRobot.tankDrive(0.4,
		 * 0.4); //what i saw was that the encoder val goes up from turning,
		 * reset that with a variable. }else{ System.out.println("finished turn
		 * step"); if(step < steps){
	...
	public boolean Rotate(double degrees) {
		...
		if(angle < minMargin){
			...
			if(minMargin - angle > 20){
				manipulators.myRobot.tankDrive(0.6, -0.6); //faster, farther
